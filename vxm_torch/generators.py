...
# modify scan_to_scan to make sure paired segmentations are yielded
# ADD: default segs input
# also modify so that the same function can be used for atlas building
# ADD: optional atlas input 
def scan_to_scan(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, segs=None, atlas=None, **kwargs):
    """
    ...
    """
    zeros = None
    # ADD: segs is passed to volgen
    gen = volgen(vol_names, batch_size=batch_size, segs=segs, **kwargs)
    while True:
        scan1 = next(gen)
        
        # ADD: do not generate a second scan if the atlas will be used instead
        if atlas is None:
            scan2 = next(gen)
        ...
        # ADD: if segs are included, gen yields a tuple, scan and segmentation
        if segs is not None:
        ...
        # always comes back as tuple, still have to unpack numpy array
        else:
        ...
        # ADD: yield paired segmentations if they were input to generator
        if segs is not None:
